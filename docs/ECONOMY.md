...
| type | value means |
|---|---|
| `guideLines` | `1` = the green guarantee lines are drawn on both sliders while the buff is active (they are hidden otherwise). Does NOT affect the event hitzones (see `eventHitzone`) |
| `eventHitzone` | `1` = the hitzone that marks the running event's face on both sliders (yellow for the banana face, purple for the disco, orange for the guitar) is drawn while the buff is active, spanning the FULL range that really hits (not a smaller "safe" marker); **without it no event hitzone is shown** (the epic Skyr) |
