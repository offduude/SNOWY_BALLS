...
## Diamond Cross, held-back event ends, music duck (branch `diamond-cross`, 2026-09-20, NOT pushed)
...
- **Music:** at the start of the miracle the theme and the event music fade out in 0.3 s (`duckMusic`) and come back in 0.3 s when the angels are done (`unduckMusic`, reads the volumes that are right then), so only the angels are heard.
