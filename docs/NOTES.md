...
- **iPhone opens the home-screen app vertically**: iOS ignores the manifest `orientation` (only Android honors it), so `landscape` there does nothing on iPhone. Fix is CSS in `index.html`: `@media (orientation: portrait) and (pointer: coarse)` rotates `#game-container` 90deg (sized `W = min(100dvh, 100vw*16/9)` pre-rotation) so the game fills a portrait phone sideways; the player turns the phone to read it. Safe because input is tap-anywhere. Desktop is untouched (`pointer: coarse` is touch-only). Verified in a 375x812 mobile emulation (fills the screen, taps register) and desktop; not on a real iPhone.
- **Phaser FIT removed** (`scale.mode = NONE`): FIT measures the container's `getBoundingClientRect()`, which is the *rotated* box on portrait phones, so it sized the canvas wrong (375x211 inside a 667x375 container). The container is already an exact CSS 16:9 box, so the canvas just fills it (`width/height: 100% !important`).
