...
## Fast throw visuals (2026-09-19) - visual only

- **What changed:** a throw that sticks to the wall no longer decelerates along a parabola to its apex. The ball now flies from the hand to the landing point at ~1100 px/s (`FLIGHT_SPEED`), nearly straight with only a small upward arc (`FLIGHT_ARC` 8% of the path, max 26px), picking up a little speed on the way (`FLIGHT_EASE` 1.25), and hits the wall at full speed, with a tiny 90 ms jolt of the game picture (`IMPACT_SHAKE`; the UI does not shake; the grenade keeps its own bigger shake). The flight lasts 0.2-0.5 s (was 0.8-0.9 s for a typical throw: measured 270 / 235 / 287 ms vs 843 / 796 / 883 ms), the camera follows more snappily for it (rate 24 instead of 14), and a spinning projectile turns the same total amount, stopping where it hits.
- **Nothing else changed:** the landing point, hit / miss, coins, marks, bounces, sounds, buffs and events are computed exactly as before from the apex physics (`launchBall`). Verified over three throws (different swing and strength): the (x, height) `finishThrow` receives equals the physics apex point to 1e-6 in every case; a W20 hit still pays, a grenade hit still explodes. The ball stayed on screen the whole flight (screen y 107-183 of 243). A throw whose apex is above the roof edge (flies over the building and falls behind it) keeps the old path - it does not stick, so there is no impact to speed up. All the numbers are constants next to `SPIN_RATE` in `main.js`. `main.js?v=100`.

- **Throw speed depends on weight; bounces follow the impact** (2026-09-19, still visual only): the flight speed is a straight line from **1600 px/s at weight 0 to 600 px/s at weight 200** (`FLIGHT_SPEED_W0/W200`, `flightSpeed(weight)` in `main.js`; snowball 100 = 1100 as before; rowan berry 1300, chestnut 1225, pinecone 970, egg 915, stone 875 - measured planned speeds equal the formula for all). The old 0.2-0.5 s clamp on the flight time is widened (0.08-1.5 s) so a heavy throw really is slower. A projectile that bounces off (chestnut, pinecone, stone, rowan berry) now leaves with a share of the velocity it hit with (`startBounce`, `flightVis.impactVX/VH` = derivative of the drawn path at its end): `BOUNCE_KEEP_X` 0.3 of the sideways speed and `BOUNCE_KEEP_UP` 0.18 of the upward speed - so a faster (lighter) throw bounces harder and the angle follows the throw: measured swing +0.3 -> vx +65 / up 179 (chestnut), swing -0.3 -> vx -65, a straight throw drops straight down (vx 0), rowan berry (fast) 67 / 194 vs stone (slow) 56 / 116. The old fixed kick (90 sideways, 130 up, random side) is gone. Landing points unchanged. `main.js?v=101`.
